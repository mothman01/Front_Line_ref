...
async function ensureAdminUser() {
  ...
  await createUser(username, password || 'ChangeMeNow!123');
  return username;
}
...
async function seedDefaultContent() {
  ...
  if ((await getContent('courses')) === null) {
    seed.courses = JSON.stringify(defaultCourses(), null, 2);
  }
  if ((await getContent('safety_rules')) === null) {
  ...
  for (const [key, value] of Object.entries(defaultContentOverrides)) {
    if ((await getContent(key)) === null) seed[key] = value;
  }

  let changed = false;
  for (const [key, value] of Object.entries(seed)) {
    if ((await getContent(key)) === null) {
